...
import AddUserToGroupHeader from 'src/components/Header/AddUserToGroupHeader';
...
import 'src/assets/sass/messagenius/pages/group/AddContactsToGroup.scss';

interface AProps {
  convId: string;
  cancelAddingUser: any;
  alreadyParticipants: any;
}

const AddContactsToGroup: React.FC<AProps> = ({
  convId,
  cancelAddingUser,
  alreadyParticipants,
}) => {
  ...
  const addContactsToGroup = () => {
    let selectedIds = selected.map(({ id }) => id);
    Parse.Cloud.run('addUsersToGroup', {
      usersToAdd: selectedIds,
      convId: convId,
    });
    cancelAddingUser(); // not cancelling, just going out of the view and going back to chatInfo
  };
  ...
  return (
    <IonPage>
      <AddUserToGroupHeader
        addContactsToGroup={addContactsToGroup}
        cancel={cancelAdding}
        selected={selected}
        hideArrowForward={hideArrowForward}
      />
                      ...
                      (user: Parse.Object<Parse.Attributes>, index: number) => {
                        return (
                          <IonItem
                            lines="none"
                            button={
                              !alreadyParticipants?.some(
                                (part: any) => part.id == user.id
                              )
                                ? true
                                : false
                            }
                          ...
                          >
                            ...
                            {alreadyParticipants?.some(
                              (part: any) => part.id == user.id
                            ) && <IonText slot="end">Participant</IonText>}
                          </IonItem>
                        );
                      }
                    )}
